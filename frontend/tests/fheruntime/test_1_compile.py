...
def test_mul(X1: float, X2: float) -> float:
    return X1 * X2
...
def test_compile() -> str:
    ...
    param_annos = {'X1': 'encrypted', 'X2': 'encrypted'}
    ...
    if test_server_api:
        ...
        compile_result = server.compile(test_mul, param_annos)
        archive_path = server.save(compile_result, compile_result.get_output_dir_path())
    else:
        inference_session = InferenceSession(onnx_file_path, param_annos, is_sim = is_sim)
