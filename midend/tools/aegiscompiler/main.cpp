...
void fhePipeline(OpPassManager &manager) {
  ...
  manager.addPass(
      createCanonicalizerPass()); // this can greatly reduce the number of
...
int main(int argc, char **argv) {
  ...
  PassPipelineRegistration<>(
      "fhe-pass", "Converts MLIR operations to FHE operations", fhePipeline);

  return asMainReturnCode(
      MlirOptMain(argc, argv, "AEGIS optimizer\n", registry));
}
