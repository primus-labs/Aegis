...
void unrollLoop(affine::AffineForOp &op, IRRewriter &rewriter)
{
    ...
    // TODO: Fix MLIR issues in mlir/Transforms/LoopUtils.h 
    // where the typedef for FuncOp is messing with the fwd declaration of FuncOp
    if (loopUnrollFull(op).failed())
    {
        emitError(op.getLoc(), "Failed to run loopUnrollFull(op)");
    }
}
