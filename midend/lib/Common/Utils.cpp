...
std::optional<std::vector<uint64_t>> extractStaticIndices(const affine::MemRefAccess &access) {
    ...
    for (size_t i = 0; i < access.getRank(); ++i) {
        ...
        accessIndices.push_back((dyn_cast<mlir::AffineConstantExpr>(thisMap.getResult(i))).getValue());
    }
...
void addGaloisIndex(mlir::ModuleOp module, int32_t newVal) {
    ...
    llvm::SmallVector<int32_t> newIndices;
    ...
    mlir::Attribute newAttr = builder.getI32VectorAttr(newIndices);
...
llvm::SmallVector<int32_t> getAllGaloisIndexs(mlir::ModuleOp module) {
    ...
    if (auto attr = module->getAttrOfType<mlir::DenseIntElementsAttr>(GALOIS_KEY_INDEX)) {
        if (attr.getElementType().isInteger(32)) {
            ...
            module.emitError("GaloisKeyIndex has invalid element type, expected i32");
        }
    }
