...
// [-10,0.01] ∪ [0.01,10] deg_list = [12, 20, 20]  epsilon = 0.01
...
inline Ciphertext<DCRTPoly> EvalCmp_gt(const Ciphertext<DCRTPoly> &x, const Ciphertext<DCRTPoly> &y) {
    ...
    Plaintext plainTwo = clientCC->MakeCKKSPackedPlaintext(std::vector(1, 2.0));
...
inline Ciphertext<DCRTPoly> EvalCmp_eq(const Ciphertext<DCRTPoly> &x, const Ciphertext<DCRTPoly> &y) {
    auto z = clientCC->EvalSub(x, y);
    auto cond = EvalCmpWithZero(z);
    Plaintext plainFour = clientCC->MakeCKKSPackedPlaintext(std::vector(1, 4.0));
...
// 1 - is_gt - is_equal => 1 - (2c^2 - c) - (4c - 4c^2) => 1 - 3c + 2c^2
inline Ciphertext<DCRTPoly> EvalCmp_lt(const Ciphertext<DCRTPoly> &x, const Ciphertext<DCRTPoly> &y) {
    auto z = clientCC->EvalSub(x, y);
    auto cond = EvalCmpWithZero(z);
    Plaintext plainOne   = clientCC->MakeCKKSPackedPlaintext(std::vector(1, 1.0));
    Plaintext plainTwo   = clientCC->MakeCKKSPackedPlaintext(std::vector(1, 2.0));
    Plaintext plainThree = clientCC->MakeCKKSPackedPlaintext(std::vector(1, 3.0));
...
// is_great + is_equal => c * (2c - 1) + 4c(1 - c) => 2c^2 - c + 4c - 4c^2 => 3c - 2c^2
...
// is_less + is_equal => (1 - 3c + 2c^2) + (4c - 4c^2) => 1 + c - 2c^2
...
RLWECipher main_graph(RLWECipher v1, RLWECipher v2) {
    auto cond = EvalCmp_gt(v1, v2);
    return EvalSel(cond, v1, v2);
}
