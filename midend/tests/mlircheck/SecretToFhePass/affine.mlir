// RUN: aegiscompiler --collect-metadata --unroll-loop-and-memory-opt --affine-simplify-structures --lower-affine --arith-to-secret --canonicalize --cse --func-to-secret --canonicalize --cse --memref-to-secret --canonicalize --cse --secret-to-fhe --canonicalize --cse < %s | FileCheck %s

module  {
  func.func  @MVP(%m: memref<16xf64> {onnx.name = "input_x", onnx.type = "encrypted"}, 
                  %v: memref<4xf64>  {onnx.name = "input_x", onnx.type = "encrypted"}
                  ) -> memref<4xf64> {
