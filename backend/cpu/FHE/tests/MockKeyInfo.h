...
struct MockKeyInfo {
    ...
    uint32_t firstModSize;              // First modulus size
    ...
    std::string scheme;                 // FHE scheme type(ckks,bfv,bgv)
};

#include "Common/Protocol.h"
#include <protocol.capnp.h>

static void _buildKeyInfo(const MockKeyInfo &mockKeyInfo, ProtoMessage<aegisprotocol::KeyInfo> &keyInfo) {
    ...
    keyInfo.asBuilder().setFirstModSize(mockKeyInfo.firstModSize);
    ...
    keyInfo.asBuilder().setScheme(mockKeyInfo.scheme);
