from primus.aegis.fheruntime import LocalFHEInferenceSession as LocalInferenceSession
import numpy as np
import os

py_dir = os.path.dirname(os.path.abspath(__file__))
onnx_file = os.path.join(py_dir, "add.onnx")

data_1 = np.array([[1, 3], [5, 7], [9, 11]], dtype=np.float32)
data_2 = np.array([[2, 4], [6, 8], [10, 12]], dtype=np.float32)

# fhe run
fhe_session = LocalInferenceSession(onnx_file)
output = fhe_session.run(["Y"], {"X1": data_1, "X2": data_2})
print("fhe run result:\n", output)
"""
[array([[ 3.,  7.],
       [11., 15.],
       [19., 23.]])]
"""

# simulate run
sim_session = LocalInferenceSession(onnx_file, is_sim = True)
output = fhe_session.run(["Y"], {"X1": data_1, "X2": data_2})
print("sim run result:\n", output)
"""
[array([[ 3.,  7.],
       [11., 15.],
       [19., 23.]])]
"""